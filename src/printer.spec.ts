...
describe('printer', () => {
    ...
    describe('printTask', () => {
        it('should print suite and nested test', () => {
            ...
            //then
            expect(console.info).toHaveBeenCalledWith(`##teamcity[testSuiteStarted name='${suite.name}' flowId='${suite.id}']`);
            expect(console.info).toHaveBeenCalledWith(`##teamcity[testSuiteStarted name='${nestedSuite.name}' flowId='${nestedSuite.id}']`);
            expect(console.info).toHaveBeenCalledWith(`##teamcity[testStarted name='${nestedTest.name}' flowId='${nestedTest.id}']`);
        ...
        it('should print skipped tests', () => {
            ...
            expect(console.info).not.toHaveBeenCalledWith(
                `##teamcity[testSuiteStarted name='${nestedSuite.name}' flowId='${nestedSuite.id}']`
            );
            expect(console.info).toHaveBeenCalledWith(
                `##teamcity[testIgnored name='${nestedTest.name}' message='' flowId='${nestedTest.id}']`
            );
            expect(taskIndex).toContain(suite);
            expect(taskIndex).toContain(nestedSuite);
            expect(taskIndex).toContain(nestedTest);
    ...
    describe('printTaskResultPack', () => {
        it('should print passt suite as finished', () => {
            ...
            //then
            expect(console.info).toHaveBeenCalledWith(`##teamcity[testSuiteFinished name='${suite.name}' flowId='${suite.id}']`);
        });
        ...
        it('shoult print passt test as finished', () => {
            ...
            //then
            expect(console.info).toHaveBeenCalledWith(
                `##teamcity[testFinished name='${test.name}' duration='${taskResult.duration}' flowId='${test.id}']`
            );
        });
        ...
        it('should print skipped test as ignored', () => {
            ...
            //then
            expect(console.info).toHaveBeenCalledWith(`##teamcity[testIgnored name='${test.name}' message='' flowId='${test.id}']`);
        });
        ...
        it('should print failed suite as failed', () => {
            ...
            //then
            expect(console.info).toHaveBeenCalledWith(
                `##teamcity[testFailed name='${test.name}' message='' details='' actual='' expected='' type='comparisonFailure' flowId='${test.id}']`
            );
        });
    });
});
